...
public:
  ...
  ~decompressed_stream_reader() { inflateEnd(&z_stream_); }

  decompressed_stream_reader(const decompressed_stream_reader &) = delete;
  decompressed_stream_reader &
  operator=(const decompressed_stream_reader &) = delete;
  ...
  std::vector<std::byte> read(std::size_t size) {
    ...
    std::vector<std::byte> result(buffer_.begin(),
                                  buffer_.begin() + bytes_to_read);
  ...
  void fill_buffer(std::size_t min_bytes) {
    while (buffer_.size() < min_bytes && !eof_compressed_) {
      if (z_stream_.avail_in == 0 && !compressed_stream_.eof()) {
        compressed_stream_.read(
            reinterpret_cast<char *>(input_chunk_buffer_.data()), CHUNK_SIZE);
        ...
        z_stream_.next_in =
            reinterpret_cast<Bytef *>(input_chunk_buffer_.data());
      }
      ...
      if (have > 0) {
        buffer_.insert(buffer_.end(), output_chunk.begin(),
                       output_chunk.begin() + have);
      }
...
std::optional<variable_length_result>
read_variable_length_size(byte_stream_reader &stream) {
  ...
  if ((first_byte & 0x80) != 0) {
    if ((first_byte & 0x40) == 0) {
      ...
      std::println(
          "Warning: Invalid first size prefix byte: {:#02x}",
          first_byte);
    ...
    if ((first_byte & 0x40) != 0) {
      ...
      prefix_bytes_read += 1;
      payload_size = ((static_cast<std::uint32_t>(first_byte) << 8) |
                      static_cast<std::uint8_t>(b1_data[0])) ^
                     0x4000;
  ...
  if (payload_size < 0) {
    std::println("Warning: Calculated negative payload size ({}). This might "
                 "indicate an issue.",
                 payload_size);
    return variable_length_result{-1, prefix_bytes_read};
  }
...
read_packet_header_from_stream(byte_stream_reader &data_stream,
                               std::uint32_t last_timestamp_ms) {
  ...
  if ((first_byte & 0x10) != 0) {
    ...
    if (ts_bytes.size() < 4) {
      std::println(
          "Warning: Unexpected EOF reading timestamp after type byte.");
      return packet_header_result{packet_type_val, timestamp_ms,
...
export void process_stream(std::istream &compressed_stream) {
    ...
    try {
      ...
      if (size_prefix_bytes.empty()) {
        if (stream.is_eof()) {
          ...
          std::println(
              "Could not read packet size prefix despite not being at EOF.");
        }
        break;
      }
      ...
      std::span<const std::byte> unused_prefix_bytes =
          prefix_stream.remaining_bytes();
      ...
      std::println("  Read size prefix ({} decomp. bytes): Expected payload "
                   "size = {} bytes",
      ...
      if (packet_data.size() != static_cast<std::size_t>(payload_size)) {
        std::println("  Warning: Incomplete packet! Expected {}, got {}.",
      ...
      std::println(stderr, "  Error during packet processing loop: {}",
                   e.what());
      break;
    }
    packet_index++;
  ...
  std::println("Total decompressed bytes processed: {}",
               total_decompressed_bytes_processed);
}
} // namespace wrpl
